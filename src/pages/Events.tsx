...
export function Events() {
  ...
  const mobileSlides = useMemo(() => chunk(filtered, EVENTS_PER_SLIDE_MOBILE), [filtered]);
    ...
    <div className="max-w-6xl mx-auto px-4 py-8">
      ...
      {filtered.length === 0 ? (
        ...
        <>
          ...
          <div
            className={`sm:hidden min-h-[200px] ${isExiting ? 'featured-exit' : isEntering ? 'featured-enter' : ''} ${isExiting || isEntering ? 'pointer-events-none' : ''}`}
            onAnimationEnd={() => {
          ...
          >
            <div className="space-y-3">
              {(mobileSlides[mobileSlide] ?? []).map((event: Event) => (
